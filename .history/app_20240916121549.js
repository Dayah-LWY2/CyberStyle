...
app.get('/search', (req, res) => {
    ...
    // If there is a search query, filter by product name or page
    if (query) {
    ...
    // Sort products based on sortBy parameter (newest, oldest, popular)
    if (sortBy) {
        switch (sortBy) {
            case 'newest':
...
app.get('/products', (req, res) => {
    const query = req.query.search || ''; // Capture the search query from the request

    // If no query is provided, return all products
    let filteredProducts = products;

    // If a search query exists, filter the products based on name or category
    if (query) {
    ...
    // Render the 'products' page with the filtered products and the search query
    res.render('products', { 
        title: 'All Products', 
        products: filteredProducts, 
        query 
