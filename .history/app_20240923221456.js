...
app.get('/confirmation', async (req, res) => {
    ...
    if (user && user.purchased.length > 0) {
        // Filter products based on purchased categories
        const purchasedProducts = user.purchased.map(code => products.find(p => p.code === code));
        const purchasedCategories = [...new Set(purchasedProducts.map(p => p.category))];

        // Find products from those categories
        recommendations.push(...products.filter(p => purchasedCategories.includes(p.category) && !user.purchased.includes(p.code)));
    }
