...
app.get('/purchased', ensureLoggedInAndExists, async (req, res) => {
    ...
    res.render('purchased', { title: 'Purchased Products', purchases: sortedPurchases, filter: null });
});

// Handle 'Receive Now' button
app.post('/receive-product', ensureLoggedInAndExists, async (req, res) => {
    ...
    // Move the product to delivered[]
    user.delivered = user.delivered || [];
    user.delivered.push(purchase);

    // Remove from purchased[]
    user.purchased = user.purchased.filter(p => p !== purchase);
...
app.get('/delivered', ensureLoggedInAndExists, async (req, res) => {
    const user = users.find(u => u.username === req.session.username);

    if (!user || !user.delivered || user.delivered.length === 0) {
        return res.status(404).send('No delivered items found.');
    }

    // Sort the delivered products from most recent to oldest
    const sortedDelivered = user.delivered.sort((a, b) => new Date(b.date) - new Date(a.date));

    res.render('delivered', { title: 'Delivered Products', deliveredItems: sortedDelivered, filter: null });
});
