...
app.get('/payment', async (req, res) => {
    ...
    if (user.rewardsPoints > 0) {
        discount = Math.min(user.rewardsPoints * 0.01, totalPrice); // Ensure price doesn't go below $30
...
app.get('/checkout', async (req, res) => {
    ...
    // Apply rewards points across the entire cart, but ensure total price doesn't drop below RM0
    let discount = 0;
    if (user.rewardsPoints > 0) {
        discount = Math.min(user.rewardsPoints * 0.01, totalPrice); // 100 points = RM1 off
        totalPrice -= discount;
    }
    ...
    res.render('checkout', {
        ...
        totalPrice,       // Pass the calculated total price
        discount,         // Pass the discount applied
        rewardsPoints: user.rewardsPoints || 0, // Pass the user's rewards points
        filter: null
    });
});
...
app.post('/process-checkout', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        if (isUsingRewards && user.rewardsPoints > 0) {
            ...
            // Check if enough points are available to deduct
        ...
        // Calculate rewards points earned (1 point per RM30 spent)
        ...
        totalSpent = Math.max(totalSpent, 0);
        console.log('Total Spent after discount:', totalSpent);

        // Update user address and clear their cart
        user.address = address;
        user.cart = [];

        // Save all changes to users.json
        await writeUsersToFile(users);
