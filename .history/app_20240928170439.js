...
app.post('/process-checkout', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        // Debugging to check useRewards and user.rewardsPoints
        console.log('useRewards (from form):', useRewards);
        console.log('isUsingRewards:', isUsingRewards);
        ...
        // Debugging user's points
        console.log('User\'s current rewards points:', user.rewardsPoints);

        let totalSpent = 0;
        let discount = 0;
        let pointsToDeduct = 0;

        // Calculate totalSpent from cart and shipping cost
        const shippingCost = 5.90;
        for (const item of cart) {
            totalSpent += item.product.price * item.quantity;
        }
        totalSpent += shippingCost;

        // Debugging totalSpent before rewards
        console.log('Total Spent Before Rewards:', totalSpent);

        // Apply rewards points if user is using them and has points
        if (isUsingRewards && user.rewardsPoints > 0) {
            discount = Math.min(user.rewardsPoints * 0.01, totalSpent); // Maximum discount allowed by points
            pointsToDeduct = Math.floor(discount * 100); // Convert RM discount to points to deduct

            // Debugging rewards application
            console.log(`Applying Discount: RM ${discount}`);
            console.log(`Points to Deduct: ${pointsToDeduct}`);

            if (user.rewardsPoints >= pointsToDeduct) {
                user.rewardsPoints -= pointsToDeduct; // Deduct the points
                console.log(`Points deducted. New Points Balance: ${user.rewardsPoints}`);
            } else {
                console.log('Not enough points to deduct');
            }

            totalSpent -= discount; // Apply the discount
        }

        totalSpent = Math.max(totalSpent, 0); // Ensure totalSpent doesn't go negative

        // Calculate points earned after purchase
        const rewardsPointsEarned = Math.floor(totalSpent); 
        user.rewardsPoints = (user.rewardsPoints || 0) + rewardsPointsEarned;

        // Debugging totalSpent and points earned
        console.log('Total Spent After Rewards:', totalSpent);
        console.log('Rewards Points Earned:', rewardsPointsEarned);
