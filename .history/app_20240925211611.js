...
app.get('/payment', async (req, res) => {
    ...
    const productInCart = cart[0]; // The only product in tempCart
    const product = products.find(p => p.code === productInCart.productCode);

    if (!product) return res.status(404).send('Product not found');

    // Calculate total price before applying discounts
    let totalPrice = product.price * productInCart.quantity;

    // Apply rewards points, but don't let the price drop below $30
    let discount = 0;
    if (user.rewardsPoints > 0) {
        discount = Math.min(user.rewardsPoints * 1, totalPrice - 30); // Ensure price doesn't go below $30
        totalPrice -= discount; // Apply the discount
    }

    // Ensure the total price doesn't go below $30
    if (totalPrice < 30) {
        totalPrice = 30;
    }
    ...
    res.render('payment', {
        ...
        cart,            // tempCart already has the product
        address,
        totalPrice,      // Pass the calculated total price
        discount,        // Pass the discount applied
...
app.post('/add-to-cart', ensureLoggedInAndExists, async (req, res) => {
    ...
    if (sessionProductIndex !== -1) {
        ...
        req.session.cart.push({ productCode, size, quantity: parseInt(quantity, 10) });
    }
...
app.post('/buy-now', ensureLoggedInAndExists, async (req, res) => {
    ...
    try {
        ...
        req.session.tempCart = [{ product, size, quantity: parseInt(quantity, 10)}];
...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        // Get the size the user selected for the product
        const selectedSize = mostRecentProduct.size;

        // Check if stock for the specific size exists
        if (!product.stock || !product.stock[selectedSize]) {
            return res.status(400).send('Invalid size or stock not available');
        }

        // Check if there is enough stock for the selected size
        if (product.stock[selectedSize] < mostRecentProduct.quantity) {
            return res.status(400).send('Insufficient stock for the selected size');
        }
        ...
        product.stock[selectedSize] -= mostRecentProduct.quantity;
         ...
         if (!product.popularity) {
            product.popularity = 0; // Initialize if it doesn't exist
        }
        product.popularity += mostRecentProduct.quantity; // Increment by the quantity purchased

        // Save updated products to file
        await writeProductsToFile(products);

        // Calculate total cost for the most recent item
        const totalSpent = product.price * mostRecentProduct.quantity;

        // Calculate rewards points
        const rewardsPointsEarned = Math.floor(totalSpent / 10);
        ...
        if (isUsingRewards && user.rewardsPoints > 0) {
            // Ensure the total spent doesn't drop below $30
            discount = Math.min(user.rewardsPoints * 1, totalSpent - 30); // Assume 1 point = RM1 discount
            totalSpent -= discount; // Reduce total spent by discount
            user.rewardsPoints -= discount; // Deduct the used points
        }

        // Ensure the minimum price is RM30
        if (totalSpent < 30) {
            totalSpent = 30;
        }
        ...
        if (!user.purchased[mostRecentProduct.productCode]) {
            user.purchased[mostRecentProduct.productCode] = 0;
        }
        user.purchased[mostRecentProduct.productCode] += mostRecentProduct.quantity; // Increment the count
...
app.post('/process-checkout', async (req, res) => {
    try {
        ...
        const totalSpent = temptCart.reduce((total, item) => {
        ...
        // Apply discount if rewards are used
        if (useRewards && user.rewardsPoints > 0) {
            const discount = Math.min(user.rewardsPoints * 1, totalSpent); // Assume 1 point = $1 discount
            totalSpent -= discount;
            user.rewardsPoints -= discount; // Deduct the used points
        }
        ...
        cart.forEach(item => {
            // Initialize if not exists
            if (!user.purchased[item.product.code]) {
                user.purchased[item.product.code] = 0;
            }
            user.purchased[item.product.code] += item.quantity; // Increment the count
        });

        // Clear the user's cart in users.json
        user.cart = [];

        // Clear the session cart and temporary cart
