...
app.post('/buy-now', ensureLoggedInAndExists, async (req, res) => {
    ...
    // Add the item to the main cart directly
    const cartItem = { 
        name: product.name, 
        code: product.code, 
        quantity: parseInt(quantity, 10),
...
app.post('/process-payment', async (req, res) => {
    // Ensure there is a cart to process
    const users = await readUsersFromFile();
    const user = users.find(u => u.username === req.session.username);

    if (!user.cart || user.cart.length === 0) return res.status(404).send('No items in cart for payment');

    // Fetch the most recent item (last added) from the cart
    const mostRecentItem = user.cart[user.cart.length - 1];

    // Process payment logic here...

    // Remove the most recent item from the main cart
    user.cart.pop(); // Remove the last item in the array
