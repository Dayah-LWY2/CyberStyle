...
app.post('/process-checkout', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        const shippingCost = 5.90;
        const totalSpentBeforeRewards = totalSpent + shippingCost; // Calculate total before any discounts
        console.log('Total Spent Before Rewards:', totalSpentBeforeRewards); // Log the correct total

        // Apply rewards points (100 points = RM1 off)
        let discount = 0;
        if (isUsingRewards && user.rewardsPoints > 0) {
            discount = Math.min(user.rewardsPoints * 0.01, totalSpentBeforeRewards); // Limit discount to total spent
            totalSpentBeforeRewards -= discount; // Reduce total spent by discount
        ...
        totalSpentBeforeRewards = Math.max(totalSpentBeforeRewards, 0);

        // Calculate rewards points earned (1 point per RM1 spent including shipping)
        const rewardsPointsEarned = Math.floor(totalSpent); // Earn points based on items only, not discounts
        user.rewardsPoints = (user.rewardsPoints || 0) + rewardsPointsEarned; // Update user points

        // Log final total after discounts
        console.log('Total Spent After Rewards:', totalSpentBeforeRewards);
