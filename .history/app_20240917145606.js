...
app.get('/payment', ensureLoggedInAndExists, async (req, res) => {
    // Retrieve the users.json file
    const users = await readUsersFromFile();
    const user = users.find(u => u.username === req.session.username);

    if (!user || !user.cart || user.cart.length === 0) {
        return res.render('payment', { cart: [] }); // Pass an empty array if no items in the cart
    }

    // Render the payment page with the main cart items
    res.render('payment', { cart: user.cart });
});
...
app.post('/process-payment', async (req, res) => {
    const username = req.session.username;
    const { address } = req.body; // Get address from form input

    const users = await readUsersFromFile();
    const user = users.find(u => u.username === username);

    if (user && user.cart.length > 0) {
        // Move the most recent item to purchased
        const mostRecentItem = user.cart.pop(); // Remove the most recent item from the cart
        user.purchased = user.purchased || [];
        user.purchased.push(mostRecentItem);

        // Store address in the user's data
