...
// Load pages data from pages.json
let pages = [];
...
const loadUsers = async () => {
    try {
        ...
        if (err.code === 'ENOENT') {
            users = []; // Initialize as empty array if file doesn't exist
        } else {
            console.error('Error reading users.json:', err);
        }
    }
};
loadUsers(); // Initialize users

// Write users data to users.json
const writeUsersToFile = async (usersData) => {
    try {
        await fs.writeFile(path.join(__dirname, 'public', 'users.json'), JSON.stringify(usersData, null, 2));
    } catch (err) {
...
app.post('/process-payment', async (req, res) => {
    try {
        ...
        // Handle payment logic (this is a placeholder - integrate with a payment gateway)
        ...
        if (userIndex !== -1) {
            ...
            // Optionally, add purchased items to user's purchase history
            users[userIndex].purchased = users[userIndex].purchased || [];
