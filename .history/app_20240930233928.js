...
app.get('/purchased', ensureLoggedInAndExists, async (req, res) => {
    ...
    const products = await readProductsFromFile();  // Ensure products are loaded

    // Pass only delivered products and products list for review lookup
    res.render('purchased', {
        title: 'Delivered Products',
        delivered: user.delivered || [], // Products that are delivered
        products: products || [],  // Ensure products is not undefined
        username // Current user's username for checking reviews
    });
});
...
app.post('/receive-product-ajax', ensureLoggedInAndExists, async (req, res) => {
    ...
    user.purchased = user.purchased.filter(p => p.productCode !== productCode);

    // Write the updated users array to file
