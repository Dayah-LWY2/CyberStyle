...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        for (const item of cart) {
            ...
            if (product) {
                product.stock -= item.quantity; // Deduct from stock
            }
        }
        ...
        await writeUsersToFile(users);
        await fs.writeFile(path.join(__dirname, 'public', 'products.json'), JSON.stringify(products, null, 2)); // Save updated stock
