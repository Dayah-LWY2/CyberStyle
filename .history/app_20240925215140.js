...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        // Calculate rewards points (1 point for every $30 spent)
        const rewardsPointsEarned = Math.floor(totalSpent / 30);
        ...
        if (isUsingRewards && user.rewardsPoints > 0) {
            // Calculate the maximum discount that can be applied without going below $30
            discount = Math.min(user.rewardsPoints, totalSpent - 30); // Ensure total doesn't go below $30
        ...
        totalSpent = Math.max(totalSpent, 30); // Ensure totalSpent is at least 30
