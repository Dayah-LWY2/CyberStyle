...
app.post('/buy-now', ensureLoggedInAndExists, async (req, res) => {
    ...
    req.session.tempCart = [cartItem]; // Temporarily store for payment processing
...
app.post('/process-payment', async (req, res) => {
    ...
    // Clear the most recent item from the main cart
    const users = await readUsersFromFile();
    const user = users.find(u => u.username === req.session.username);

    // Remove the most recent item from the main cart
    user.cart = user.cart.filter(item => !(item.product.code === tempCart[0].product.code && item.size === tempCart[0].size));

    // Move the purchased item to the purchased section
    user.purchased = user.purchased || [];
    user.purchased.push(tempCart[0]);
