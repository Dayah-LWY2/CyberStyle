...
app.post('/process-payment', async (req, res) => {
    try {
        ...
        if (userIndex !== -1) {
            ...
            // Add product codes from tempCart to the purchased array
...
app.post('/signup', async (req, res) => {
    ...
    users.push({ username, email, password, phone, gender, dob, purchased: [] });
    await writeUsersToFile(users);
...
// Buy now route (adds to tempCart)
app.post('/buy-now', ensureLoggedInAndExists, (req, res) => {
    ...
    req.session.tempCart = [{
        ...
        size,
        product: { price: product.price, code: product.code } // Store code
    }];
