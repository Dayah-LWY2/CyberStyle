...
app.post('/products/:id/review', ensureLoggedInAndExists, async (req, res) => {
    ...
    if (product) {
        ...
        if (!user || !user.purchased.includes(productId)) {
    ...
    try {
        ...
        if (userIndex !== -1) {
            ...
            tempCart.forEach(item => {
                users[userIndex].purchased.push(item.product.id);
            });
