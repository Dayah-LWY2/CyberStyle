...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        const rewardsPointsEarned = Math.floor(totalSpent); // 1 point for every $1 spent
        ...
        if (isUsingRewards && user.rewardsPoints > 0) {
            const redeemableDiscount = Math.floor(user.rewardsPoints / 100); // $1 for every 100 points
            discount = Math.min(redeemableDiscount, totalSpent); 
            totalSpent -= discount; // Reduce total spent by discount
            user.rewardsPoints -= discount * 100; // Deduct the used points (100 points per $1)
        }
