...
app.get('/cart', async (req, res) => {
    ...
    const cart = user.cart || []; // Get user's cart from users.json
    res.render('cart', { title: 'Cart', cart, filter: null });
});
...
app.post('/process-checkout', async (req, res) => {
    try {
        const { address } = req.body;
        if (!address || address.trim() === '') return res.status(400).send('Address is required');
        ...
        if (cart.length === 0) return res.status(404).send('No items in cart for payment');

        // Add items from cart to purchased
        user.purchased = user.purchased || [];
        cart.forEach(item => user.purchased.push(item.product.code));
