...
app.get('/checkout', async (req, res) => {
    const username = req.session.username;  
    const users = await readUsersFromFile();
    const products = await readProductsFromFile();  // Load products array
    const user = users.find(u => u.username === username);
    const address = user ? user.address || '' : '';
    ...
    // Calculate total price before applying any discounts
    let totalPrice = 0;
    ...
    // Apply rewards points, but ensure the price doesn’t go below $30
    let discount = 0;
    if (user.rewardsPoints > 0) {
        discount = Math.min(user.rewardsPoints/100, totalPrice);
        totalPrice -= discount;
    }

    // Ensure the total price doesn't drop below 0
    totalPrice = Math.max(totalPrice, 0);

    // Render the checkout page
    res.render('checkout', {
        title: 'Checkout',
        cart,             // Pass the user's cart items
        address,          // Pass the user's address
        totalPrice,       // Pass the calculated total price
        discount,         // Pass the discount applied
        rewardsPoints: user.rewardsPoints || 0, // Pass available rewards points
        filter: null
    });
});
...
app.post('/process-checkout', async (req, res) => {
    try {
        const { address, useRewards } = req.body; // Include useRewards in the request body
        const isUsingRewards = useRewards === 'true'; // Check if rewards are being used
        ...
        let totalSpent = 0; // Initialize totalSpent
        ...
        for (const cartItem of cart) {
            ...
            // Check if stock for the specific size exists and if there's enough stock
            if (!product.stock || !product.stock[selectedSize] || product.stock[selectedSize] < cartItem.quantity) {
                return res.status(400).send(`Insufficient stock for size ${selectedSize}`);
            }

            // Decrease the stock of the product for the specific size
            product.stock[selectedSize] -= cartItem.quantity;

            // Increment the popularity count for the product
            product.popularity = (product.popularity || 0) + cartItem.quantity;

            // Add the total cost for this item
            totalSpent += product.price * cartItem.quantity;

            // Update the user's purchased products
            user.purchased = user.purchased || {};
            user.purchased[cartItem.productCode] = (user.purchased[cartItem.productCode] || 0) + cartItem.quantity;
        }
        ...
        let discount = 0;
        if (isUsingRewards && user.rewardsPoints > 0) {
            discount = Math.min(user.rewardsPoints/100, totalSpent); // 1 point = $0.01
            totalSpent -= discount; // Reduce total spent by discount
            user.rewardsPoints -= discount * 100; // Deduct used points (100 points = $1.00)
        }

        // Ensure the total spent is non-negative
        totalSpent = Math.max(totalSpent, 0);
        ...
        // Save updated product stock and popularity
        await writeProductsToFile(products);
        
        // Save the updated user data back to the users.json file
        await writeUsersToFile(users);

        // Clear the session cart after processing the checkout
        req.session.tempCart = [];
        req.session.cart = [];
