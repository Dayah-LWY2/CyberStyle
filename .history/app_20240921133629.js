...
app.get('/product/:code', async (req, res) => {
    ...
    res.render('product', { title: 'Product', product, users, loggedInUser, filter: null });
});
...
app.post('/product/:code/review', ensureLoggedInAndExists, async (req, res) => {
    ...
    const product = products.find(p => String(p.code) === String(req.params.code));
    ...
    if (product) {
        ...
        if (!user || !user.purchased.includes(String(productCode))) {
...
app.post('/process-payment', async (req, res) => {
    try {
        ...
        console.log('Processing payment for tempCart:', tempCart);
        ...
        if (userIndex !== -1) {
            ...
            tempCart.forEach(item => {
                if (item.product && item.product.code) {
                    users[userIndex].purchased.push(String(item.product.code));
                } else {
        ...
        res.status(500).send('An error occurred while processing your payment. Please try again later.');
    }
});
...
app.post('/process-checkout', async (req, res) => {
    try {
        ...
        if (userIndex !== -1) {
            ...
            cart.forEach(item => {
                if (item.product && item.product.code) {
                    users[userIndex].purchased.push(item.product.code); // Ensure the product code is stored
                } else {
                    console.error('Invalid product or missing product code:', item);
                }
            });
