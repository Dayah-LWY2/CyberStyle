...
// Function to get user spending asynchronously
const getUserSpent = async (username) => {
    const usersData = await readUsersFromFile(); // Read users data
    const user = usersData.find(user => user.username === username);
    if (!user || !user.purchased) return 0;

    let totalSpent = 0;
    for (const [productCode, quantity] of Object.entries(user.purchased)) {
        const product = products.find(prod => prod.code === productCode);
        if (product) {
            totalSpent += product.price * quantity; // Calculate total spending based on quantity
        }
    }
    return totalSpent;
};

// Function to determine the user's tier based on spending
const getUserTier = (totalSpent) => {
    if (totalSpent > 1000) return { tier: 4, discount: 50 };
    if (totalSpent > 500) return { tier: 3, discount: 30 };
    if (totalSpent > 300) return { tier: 2, discount: 15 };
    if (totalSpent > 100) return { tier: 1, discount: 10 };
    return { tier: 0, discount: 0 }; // No tier
};

app.get('/rewards', async (req, res) => {
    const username = req.query.username; // Get username from query parameters
    const totalSpent = await getUserSpent(username); // Get total spent by the user
    const tierInfo = getUserTier(totalSpent); // Get tier and discount info

    res.render('rewards', { title: 'Rewards', totalSpent, tierInfo, filter: null }); // Render rewards page with user spending and tier info
});
...
app.get('/cart', ensureLoggedInAndExists, async (req, res) => {
    ...
    res.render('cart', { title: 'Cart', cart: detailedCart, filter: null });
});
...
app.get('/confirmation', async (req, res) => {
    ...
    if (user && Object.keys(user.purchased).length > 0) {
        const purchasedCategories = [];
        for (const code in user.purchased) {
            const product = products.find(p => p.code === code);
            if (product) {
                purchasedCategories.push(product.category);
            }
        }

        recommendations.push(...products.filter(p => purchasedCategories.includes(p.category) && !user.purchased[p.code]));
    }

    const recommendedProducts = products.filter(product => {
        return !user.purchased.includes[product.code] && product.category !== 'new';
    });

    res.render('confirmation', { title: 'Confirmation', recommendations, recommendedProducts, filter: null });
});
...
app.get('/autocomplete', (req, res) => {
    ...
    const matchedProducts = products.filter(product => 
        product.name.toLowerCase().includes(query) && !product.categories.includes('new')
    );
...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        // Remove the most recent product (last product in the cart)
