...
app.post('/product/:code/review', ensureLoggedInAndExists, async (req, res) => {
    ...
    if (product) {
        ...
        const existingReview = product.reviews && product.reviews.find(review => review.username === req.session.username);
        if (existingReview) {
            return res.status(403).send('You have already reviewed this product.');
        }
