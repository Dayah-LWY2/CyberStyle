...
app.get('/confirmation', async (req, res) => {
    const username = req.session.username;
    const users = await readUsersFromFile(); // Function to read users from a file
    const user = users.find(u => u.username === username);
    const recommendations = [];

    if (user && Object.keys(user.purchased).length > 0) {
        const purchasedCategories = new Set(); // Use Set to avoid duplicates
        ...
        recommendations.push(...products.filter(p => purchasedCategories.has(p.category)));
    }
