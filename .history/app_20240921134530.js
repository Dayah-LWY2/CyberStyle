...
app.post('/products/:code/review', ensureLoggedInAndExists, async (req, res) => {
        ...
        if (userIndex !== -1) {
            ...
            tempCart.forEach(item => {
                if (item.product && item.product.code) {
                    users[userIndex].purchased.push(item.product.code); // Add the product code to the purchased array
                } else {
