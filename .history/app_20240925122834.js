...
app.get('/confirmation', async (req, res) => {
    ...
    if (user && Object.keys(user.purchased).length > 0) {
        ...
        recommendations.push(...products.filter(p => purchasedCategories.includes(p.category)));
    }

    res.render('confirmation', { title: 'Confirmation', recommendations, recommendations, filter: null });
});
...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        if (isUsingRewards && user.rewardsPoints > 0) {
            discount = Math.min(user.rewardsPoints * 1, totalSpent); // Assume 1 point = $1 discount
