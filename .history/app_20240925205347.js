...
app.get('/payment', async (req, res) => {
    ...
    const mostRecentProduct = cart[cart.length - 1]; // Get the last added item
    const product = products.find(p => p.code === mostRecentProduct.productCode);
    
    if (!product) return res.status(404).send('Product not found');

    let totalPrice = product.price * mostRecentProduct.quantity;

    // Apply rewards points, but don't let the price go below $30
    let discount = 0;
    if (user.rewardsPoints > 0) {
        discount = Math.min(user.rewardsPoints * 1, totalPrice - 30); // Ensures the price doesn't go below $30
        totalPrice -= discount;
    }

    // Ensure the displayed price is at least $30
    if (totalPrice < 30) {
        totalPrice = 30;
    }
    ...
    res.render('payment', {
        ...
        cart: [mostRecentProduct], // Assuming single product in the cart
        address,
        totalPrice,
        discount,
