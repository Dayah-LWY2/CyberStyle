...
const getRecommendations = (user, products) => {
    const purchasedCodes = user.purchased;
    const purchasedProducts = products.filter(p => purchasedCodes.includes(p.code));

    const recommendedProducts = new Set();

    purchasedProducts.forEach(product => {
        products.forEach(p => {
            if (p.code !== product.code && (p.category === product.category || p.tags.some(tag => product.tags.includes(tag)))) {
                recommendedProducts.add(p);
            }
        });
    });

    return Array.from(recommendedProducts).slice(0, 4); // Return top 4 recommendations
};
...
app.get('/product/:code', async (req, res) => {
    ...
    const priceRangeProducts = products.filter(p => p.price >= minPrice && p.price <= maxPrice && p.code !== product.code);

    res.render('product', { title: 'Product', product, users, loggedInUser, hasReviewed, priceRangeProducts, filter: null });
});
...
app.get('/confirmation', async (req, res) => {
    ...
    // Get unique categories from purchased products
    const purchasedProducts = products.filter(product => purchasedCodes.includes(product.code));
    const categories = [...new Set(purchasedProducts.map(product => product.category))];

    // Find related products in the same categories
    const recommendedProducts = products.filter(product => categories.includes(product.category) && !purchasedCodes.includes(product.code));

    res.render('confirmation', { title: 'Confirmation', recommendedProducts, filter: null });
});
...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        // Add the removed product to the user's purchased list
        if (!user.purchased) {
            user.purchased = [];
        }
        user.purchased.push(mostRecentProduct.productCode);
        ...
        // Clear the session cart after processing the payment
        req.session.cart = [];
