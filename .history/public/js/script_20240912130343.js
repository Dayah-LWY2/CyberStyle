...
function showPopup(popupId) {
    ...
    if (popup) {
        ...
        }, 2000); // Adjusted to hide after 2 seconds
    }
}
...
window.onload = function() {
    ...
    if (urlParams.has('status')) {
        ...
        console.log("Status found: ", status); // Debugging log

        // Show the correct popup based on the status
        switch (status) {
            case 'logged-out':
        ...
        // Immediately remove 'status' from the URL to prevent it from persisting
        const newUrl = window.location.pathname;  // This removes the query parameters completely
