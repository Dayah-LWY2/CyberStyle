...
document.getElementById('size-select').addEventListener('change', updateStockDisplay);
...
function updateStockDisplay() {
    ...
    var stockData = `<%- JSON.stringify(product.stock) %>`; // Embed the product stock data
    var stockCountElement = document.getElementById('stock-count');

    // Get the stock for the selected size and display it
    var stockAvailable = stockData[selectedSize] || 0; // Fallback to 0 if not found
    stockCountElement.innerText = stockAvailable;

    // Update the quantity input's max stock
    var qtyInput = document.getElementById('quantity');
    qtyInput.setAttribute('data-stock', stockAvailable);

    // Ensure the quantity does not exceed available stock
    if (parseInt(qtyInput.value) > stockAvailable) {
        qtyInput.value = stockAvailable; // Set quantity to available stock
    }
}

// Initial stock display on page load
updateStockDisplay();
