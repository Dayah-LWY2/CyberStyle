...
async function receiveProduct(productCode, purchaseDate) {
    try {
        ...
        if (data.success) {
            ...
            `;
            const deliveryInfoHtml = `Delivered on ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`;

            // Update only the button and delivery status
            productElement.querySelector('.purchased-action').innerHTML = buttonHtml;
