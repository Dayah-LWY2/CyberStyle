...
function updateTotal() {
    ...
    const products = JSON.parse(document.getElementById('tempCartData').textContent); // Get tempCart data from DOM
  
    // Calculate total
    let total = products.reduce((acc, item) => acc + (item.quantity * item.product.price), 0) + shippingCost;
  
    // Reduce total if rewards are applied
    if (useRewards) {
      const rewardsToUse = Math.min(parseFloat(document.getElementById('rewardsPoints').textContent), total);
      total -= rewardsToUse;
    }
