...
const imageContainer = document.querySelector('.product-image'); // The main image container
    const mainImage = document.getElementById('main-product-image'); // The main product image
    let isZoomed = false; // State variable to track zoom status

    // Event listener for clicking on the main image to zoom in/out
    imageContainer.addEventListener('click', function () {
        if (!isZoomed) {
            mainImage.style.transform = 'scale(2)'; // Zoom in
            imageContainer.style.cursor = 'zoom-out'; // Change cursor to indicate zoom out
            isZoomed = true; // Update zoom state
        } else {
            mainImage.style.transform = 'scale(1)'; // Zoom out
            imageContainer.style.cursor = 'zoom-in'; // Change cursor to indicate zoom in
            isZoomed = false; // Update zoom state
        }
    });

    // Event listener for moving the mouse to slide the image when zoomed
    imageContainer.addEventListener('mousemove', function (e) {
        if (isZoomed) {
            const containerRect = imageContainer.getBoundingClientRect(); // Get container dimensions
            const imageRect = mainImage.getBoundingClientRect(); // Get main image dimensions

            // Calculate mouse position relative to the container
            const mouseX = e.clientX - containerRect.left;
            const mouseY = e.clientY - containerRect.top;

            // Calculate offset for the zoomed image
            const offsetX = (mouseX / containerRect.width) * (imageRect.width - containerRect.width);
            const offsetY = (mouseY / containerRect.height) * (imageRect.height - containerRect.height);

            // Boundary checks to keep the image within the container
            const maxOffsetX = Math.max(0, imageRect.width - containerRect.width);
            const maxOffsetY = Math.max(0, imageRect.height - containerRect.height);

            const boundedOffsetX = Math.min(Math.max(offsetX, 0), maxOffsetX);
            const boundedOffsetY = Math.min(Math.max(offsetY, 0), maxOffsetY);

            // Move the image within the boundaries
            mainImage.style.transform = `scale(2) translate(${-boundedOffsetX}px, ${-boundedOffsetY}px)`;
        }
    });