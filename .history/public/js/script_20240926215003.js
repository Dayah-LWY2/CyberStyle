...
function updateTotal() {
    ...
    const rmDeductedElement = document.getElementById('rmDeducted');
    const pointsDeductedElement = document.getElementById('pointsDeducted');
    ...
    const rewardsPoints = parseFloat(rewardsPointsElement.innerText) || 0;
    ...
    // Calculate how many points the user will earn (1 point per RM spent)
    const pointsEarned = Math.floor(totalAmount); // Each RM earns 1 point
    pointsAddedElement.innerText = pointsEarned; // Show points to be added

    // Apply rewards points if checkbox is checked
    if (useRewardsCheckbox.checked) {
        const pointsToDeduct = Math.min(rewardsPoints, Math.floor(totalAmount * 100)); // Max points usable in terms of RM (100 points = RM1)
        const rmDeducted = pointsToDeduct * 0.01; // RM deducted (1 point = RM0.01)
        const updatedTotal = totalAmount - rmDeducted;
        ...
        // Display RM deducted and points deducted
        rmDeductedElement.innerText = rmDeducted.toFixed(2); // RM deducted
        pointsDeductedElement.innerText = pointsToDeduct; // Points used

        // Update remaining rewards points display
        rewardsPointsElement.innerText = (rewardsPoints - pointsToDeduct).toFixed(0); // Remaining points
    } else {
        // No rewards applied, reset the total
        totalAmountElement.innerText = totalAmount.toFixed(2);
        // Hide the RM and points deductions when unchecked
        rmDeductedElement.innerText = '0.00';
        pointsDeductedElement.innerText = '0';
    }
}
