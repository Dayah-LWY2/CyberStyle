...
document.addEventListener("DOMContentLoaded", function() {
    ...
    filterDropdown.querySelectorAll('a').forEach(filterLink => {
        filterLink.addEventListener('click', (event) => {
            event.preventDefault();
            const filter = filterLink.getAttribute('data-filter');
            searchInput.dataset.filter = filter; // Store selected filter in the input field
            searchForm.submit(); // Submit the form with the filter
        });
    }); 
