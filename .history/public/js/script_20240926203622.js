...
const originalTotal = parseFloat(document.getElementById('totalAmount').innerText) || 0; // Ensure fallback to 0
...
function updateTotal() {
    ...
    let totalAmount = 0; // Initialize total from original total
    const rewardsPoints = parseFloat(rewardsPointsElement.innerText) || 0; // Ensure fallback to 0

    // Recalculate the total based on cart items
    cartItems.forEach(item => {
        // Ensure you access the right cells
        const priceText = item.cells[3].innerText.replace('RM ', '').trim(); // Unit Price (4th cell)
        const quantityText = item.cells[2].innerText.trim(); // Quantity (3rd cell)
        ...
        if (!isNaN(price) && !isNaN(quantity)) {
            totalAmount += price * quantity; // Add to total
        }
    });
    ...
    // Calculate discount based on rewards points (100 points = RM1)
    const discount = useRewardsCheckbox.checked ? Math.min(rewardsPoints * 0.01, totalAmount) : 0;

    const updatedTotal = totalAmount - discount; // Update total after applying discount

    // Ensure the total does not go below RM0
    const finalTotal = Math.max(updatedTotal, 0);

    // Update the displayed total
    totalAmountElement.innerText = finalTotal.toFixed(2); // Format to 2 decimal places
}

// Call this function on checkbox change event
document.getElementById('useRewards').addEventListener('change', updateTotal);

// Run the function initially to reflect any default values
updateTotal();
