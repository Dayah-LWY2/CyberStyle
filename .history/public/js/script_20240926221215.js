...
function updateTotal() {
    ...
    // Display RM deduction and points deducted if checkbox is checked
    if (useRewardsCheckbox.checked) {
        deductionElement.innerText = `RM ${discount.toFixed(2)}`; // Display RM deducted
        pointsDeductedElement.innerText = pointsDeducted; // Display points deducted
    } else {
        deductionElement.innerText = ''; // Clear if not checked
        pointsDeductedElement.innerText = ''; // Clear if not checked
    }
    
    // Always display points added
    const pointsAdded = Math.floor(rewardsPoints / 30); // Example calculation for points added
