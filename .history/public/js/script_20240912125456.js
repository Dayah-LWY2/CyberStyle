...
window.onload = function() {
    ...
    if (urlParams.has('status')) {
        ...
        // Add a short delay before removing the 'status' from the URL
        setTimeout(() => {
            console.log("Removing status from URL"); // Debugging log
            const newUrl = window.location.pathname;
            window.history.replaceState({}, '', newUrl);
        }, 100); // 100ms delay to allow pop-up to show properly
    }
};
