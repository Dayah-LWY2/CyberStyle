...
function updateTotal() {
    ...
    let totalAmount = originalTotal; // Initialize total from original total
    const rewardsPoints = parseFloat(rewardsPointsElement.innerText); // Get available rewards points

    // Recalculate the total based on cart items
    totalAmount = 0; // Reset total
    cartItems.forEach(item => {
        const price = parseFloat(item.cells[3].innerText.replace('RM ', '')); // Unit Price (4th cell)
        const quantity = parseInt(item.cells[2].innerText); // Quantity (3rd cell)
