...
function updateTotal() {
    ...
    // Determine which cart is available (checkout or payment)
    ...
    const pointsToAdd = Math.floor(totalAmount / 30);
    pointsAddedElement.innerText = pointsToAdd; // Points earned based on spending

    // Apply rewards points if checkbox is checked
    if (useRewardsCheckbox.checked) {
        const pointsToDeduct = Math.min(rewardsPoints, Math.floor(totalAmount)); // Points redeemable
        const rmDeducted = pointsToDeduct * 0.01; // RM deducted based on points
        ...
        // Show points deducted and RM deducted
        pointsDeductedElement.innerText = pointsToDeduct; // Points used
        rmDeductedElement.innerText = rmDeducted.toFixed(2); // RM deducted
        document.getElementById('pointsDeductionSection').style.display = 'block'; // Show deduction info
    } else {
        // No rewards applied, reset the display
        totalAmountElement.innerText = totalAmount.toFixed(2);
        document.getElementById('pointsDeductionSection').style.display = 'none'; // Hide deduction info
    }
}
