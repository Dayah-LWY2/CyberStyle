...
app.post('/receive-product-ajax', ensureLoggedInAndExists, async (req, res) => {
    ...
    // Do not move it to delivered[], just update the status in purchased[]
...
app.post('/update-cart', ensureLoggedInAndExists, async (req, res) => {
    const { productCode, size, quantity } = req.body;
    ...
    const cartItem = user.cart.find(item => item.productCode === productCode && item.size === size);
    ...
    if (cartItem) {
        ...
        user.cart.push({ productCode, size, quantity });
    }
