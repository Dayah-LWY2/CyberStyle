...
async function getPurchasedItems(username) {
    const users = await readUsersFromFile();
    const user = users.find(u => u.username === username);
    
    if (!user || !user.purchased) {
        return []; // Return an empty array if no purchased items
    }

    const products = await readProductsFromFile();

    // Retrieve the product details based on purchased items
    const purchasedProducts = Object.keys(user.purchased).map(productId => {
        const product = products.find(p => p.code === productId);
        if (product) {
            return {
                ...product,
                quantity: user.purchased[productId]
            };
        }
        return null;
    }).filter(p => p !== null);
...
app.get('/purchased', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        // Check if the user exists and has purchased items
        if (!user || !user.purchased) {
            return res.render('purchased', { purchasedItems: [] }); // Render with empty array
        }

        // Transform purchased items into an array for easier rendering
        const purchasedItems = Object.keys(user.purchased).map(productCode => {
            const product = products.find(p => p.code === productCode);
            return {
                code: productCode,
                name: product ? product.name : 'Unknown Product', // Fallback if product is not found
                quantity: user.purchased[productCode]
            };
        });

        res.render('purchased', { purchasedItems });
    } catch (error) {
...
app.post('/signup', async (req, res) => {
    ...
    users.push({ username, email, password, phone, gender, dob, purchased: {}, cart: [], deliveredProducts: [] }); // Initialize purchased as an empty array
    await writeUsersToFile(users);
...
app.post('/process-payment', ensureLoggedInAndExists, async (req, res) => {
    try {
        ...
        let totalSpentBeforeRewards = totalSpent; // Change const to let
...
app.post('/confirm-delivery/:productCode', async (req, res) => {
    ...
    if (!user.deliveredProducts) {
        user.deliveredProducts = [];
    }

    if (!user.deliveredProducts.includes(productCode)) {
        user.deliveredProducts.push(productCode);
